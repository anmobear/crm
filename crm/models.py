...
class UserProfile(User):
    ...
    用户表
    """
    username = models.EmailField(verbose_name="账号", max_length=255, unique=True, )
...
class Customer(models.Model):
    ...
    def show_class(self):
        en =  "|".join([str(i) for i in self.class_list.all()])    # 循环班级的对象 用str()取值 用join 拼接
        return en
    '''
    ('signed', "已报名"),
    ('unregistered', "未报名"),
    ...
    def show_stutas(self):
        ...
        return mark_safe(f"<span class='span' style=' background-color:{stutas.get(self.status)}; color:white;padding: 8px '>{self.get_status_display()}</span>")
    def __str__(self):
        return f"QQ:{self.qq},姓名:{self.name}"
...
class ClassList(models.Model):
    ...
    def __str__(self):
        # print(self.teachers.all())
        return f"{self.get_course_display()}-{self.semester}({self.campuses.name})"
...
class CourseRecord(models.Model):
    ...
    def show(self):
        return f"{self.re_class} {self.day_num}"

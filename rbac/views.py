...
def role_change(request, role_id=None):
    obj = models.Role.objects.filter(pk=role_id).first()
    from_obj = forms.RoleFrom(instance=obj)
    if request.method == "POST":
        from_obj = forms.RoleFrom(request.POST, instance=obj)
        if from_obj.is_valid():
            from_obj.save()
            return redirect(reverse('rbac:role_list'))
    return render(request, "rbac/role_add.html", {"from_obj": from_obj})


def delete(request, name, del_id):
    class_obj = getattr(models, name.capitalize())
    class_obj.objects.filter(pk=del_id).delete()
    return redirect(f'{name}_list')
...
def menu_list(request):
    ...
    if mid:
        all_permission = models.Permission.objects.filter(Q(parent__menu_id=mid) | Q(menu_id=mid)).values(
            "name", "url", "title", "menu__title", "parent_id", "id")
    else:
        all_permission = models.Permission.objects.all().values(
            "name", "url", "title", "menu__title", "parent_id", "id")
    permission_dict = {}
    for i in all_permission:  # 构建数据结构
...
def menu_change(request, menu_id=None):
    obj = models.Menu.objects.filter(pk=menu_id).first()
    from_obj = forms.MenuFrom(instance=obj)
    if request.method == "POST":
        from_obj = forms.MenuFrom(request.POST, instance=obj)
        if from_obj.is_valid():
            from_obj.save()
            return redirect(reverse('rbac:menu_list'))
    return render(request, "rbac/from.html", {"from_obj": from_obj})
